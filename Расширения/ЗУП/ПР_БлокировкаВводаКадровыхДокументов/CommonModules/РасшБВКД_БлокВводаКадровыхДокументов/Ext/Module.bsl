...
Процедура РасшБВКД_ОбработкаПроверкиЗаполненияКД(Источник, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если Источник.ПериодРегистрации < НачалоМесяца(Источник.Дата) Тогда
		...
		Сообщение.Текст = "Дата документа не может наступать позже периода регистарции (месяца)!";
		Сообщение.Сообщить();
		Отказ = Истина;
		Возврат;	
	КонецЕсли;
	...
	Если Отказ Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Ошибка получения списка сотрудников из документа.";
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НачислениеЗарплаты.Проведен = ИСТИНА
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Проведение документа невозможно, уже есть рассчитанный документ 'Начисление зарплаты и взносов' ";
		Сообщение.Сообщить();
		Отказ = Истина; 
		Возврат;
	КонецЕсли;       
