...
Процедура РасшБВКД_ОбработкаПроверкиЗаполненияКД(Источник, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ПериодРегистрации", Источник.Метаданные()) Тогда
	...
	ИначеЕсли  Источник.Метаданные().Реквизиты.Найти("СовмещающийСотрудник") <> Неопределено Тогда
		
		СписокСотрудников.Добавить(Источник.СовмещающийСотрудник);
		
	ИначеЕсли Источник.Метаданные().ТабличныеЧасти.Найти("ДанныеОВремени") <> Неопределено Тогда
		
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Сотрудник", Источник.Метаданные().ТабличныеЧасти.ДанныеОВремени) Тогда
			
			Для каждого СтрокаТЧ Из Источник.ДанныеОВремени Цикл
